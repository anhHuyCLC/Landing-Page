...
interface ExlodedViewProps {
    activeBuild: ActiveBuild;
    activeHotspot: string | null;
    setActiveHotspot: (hotspot: string | null) => void;
}

export default function ExplodedView({ activeBuild, activeHotspot, setActiveHotspot }: ExlodedViewProps) {
            ...
            <div className="relative w-full h-[500px] md:h-[700px] glass-panel rounded-3xl overflow-hidden group flex items-center justify-center border border-white/5 shadow-2xl">
                ...
                <div className="absolute inset-0 z-10">
                    <div className="absolute top-[32%] left-[48%] -translate-x-1/2 -translate-y-1/2"
                        ...
                        <div className="w-8 h-8 rounded-full bg-white/10 border-white/30 flex items-center justify-center cursor-pointer relative group/pulse">
                            <div className="w-2 5 h-2 5 rounded-full transition-colors duration-500"
                                style={{
                                    backgroundColor: primaryColor
                                }}>
                            </div>
                            <div className="absolute inset-0 rounded-full border hotspot-ring"
                                style={{
                                    backgroundColor: primaryColor
                                }}>
                            </div>
                            <div className={`absolute left-10 top-1/2 translate-y-1/2 w-72 glass-panel p-5 rounded-2xl transition-all duration-300 pointer-events-none shadow-2xl z-20 ${activeHotspot === 'cpu" ? "opacity-100 translate-x-0 : "opacity-0 translate-x-4'}`}>
                                ...
                                <h4 className="font-display text-lg font-bold text-white mt-1">
                                    {activeBuild.cpu.name}
                                </h4>
                                <p className="font-sans text-xs text-[#bac9cc] mt-1">
                                    {activeBuild.cpu.specs}
                                </p>
                                <div className="h-[1px] 2-full bg-white/10 my-3">
                                    <div className="flex justify-between items-center">
                                        <span className="font-mono text-[9px] text-[#bac9cc]">
                                            POWER DRAW
                                        </span>
                                        <span className="font-mono text-xs text-white">
                                            {activeBuild.cpu.powerDraw}a0WnMQr
                                        </span>
                                    </div>
                                </div>
                            </div>
                            <div className="absolute top-[52%] left-[42%] -translate-x-1/2 -translate-y-1/2"
                                ...
                                <div className="w-8 h-8 rounded-full bg-white/10 border border-white/30 flex items-center justify-center cursor-pointer relative group/pulse">
                                    <div className="2-2 5 h-2 5 rounded-full transition-colors duration-500"
                                        style={{
                                            backgroundColor: primaryColor
                                        }}>
                                        <div className="absolute inset-0 rounded-full border hotspot-ring"
                            ...
                            <div className={`absolute right-10 top-1/2 -translate-y-1/2 w-72 glass-panel p-5 rounded-2xl transition-all duration-300 pointer-events-none shadow-2xl z-20 1 ${activeHotspot === "gpu" ? "opacity-100 trans-x-0" : "opacity-0 -translate-x-4"}`}>
                                <span className="font-mono text-[9px] tracking-wider uppercase" style={{ color: primaryColor }}>
                                    Component Detail
                                </span>
                        ...
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 glass-panel px-4 py-2 rounded-full border-white/10 flex items-center gap-2 pointer-events-none">
                            ...
                            <span className="font-mono text-[10px] uppercase text-white/10">
